              ...
              <div className="hidden md:block">
                ...
                {navLinks.map(({ href, label }) => (
                    <Link
                        ...
                        key={`${href}${label}`}
                        href={href}
                        className="hover:bg-[#e3282c] hover:text-white font-medium text-[#e3282c] px-3 py-2 rounded-md text-sm"
                    >
                        {label}
                    </Link>
                    ))}
        ...
        <div className={`md:hidden ${isMobileMenuOpen ? 'block' : 'hidden'}`} id="mobile-menu">
          ...
          {navLinks.map(({ href, label }) => (
                    <Link
                        key={`${href}${label}`}
                        href={href}
                        className="hover:bg-[#e3282c] hover:text-white  text-[#e3282c] block px-3 py-2 rounded-md text-base font-medium"
                        onClick={toggleMobileMenu}
                    >
                        {label}
                    </Link>
